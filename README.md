...
**Known limitations:**
- **"Always Allow" persists per-project** — it writes a rule into the current project's `.claude/settings.local.json` `permissions.allow` (the same file/array Claude Code's own "don't ask again" uses), scoped exactly as narrowly: one exact rule per Bash subcommand, or one rule for the specific file on Write/Edit/NotebookEdit — never a wildcard the tap didn't visibly approve. The hook checks that file itself on every call and answers instantly without bothering the device once something's covered; it doesn't rely on Claude Code noticing the file mid-session. Scope is per-project only — tapping Always in one project never grants anything in another.
