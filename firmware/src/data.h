...
// field), the UI views and the alert melodies. INPUT..DONE and CLEAR come
// from the daemon; LIMIT and RESET are raised by the firmware itself.
enum : uint8_t {
    ATTN_NONE  = 0,
    ATTN_INPUT = 1,   // Claude is waiting for an answer
    ATTN_PERM  = 2,   // permission prompt
    ATTN_DONE  = 3,   // turn finished
    ATTN_LIMIT = 4,   // session limit almost exhausted (firmware-local)
    ATTN_RESET = 5,   // session window reset (firmware-local)
    ATTN_CLEAR = 6,   // dismiss the attention view, no sound
};
