// CYD variant of splash.cpp.
//
// Same public API and animation logic as the AMOLED splash, but:
//   - 20x20 grid scaled 12x to 240x240 (centered on the 320x240 panel)
//   - Canvas buffer allocated from internal SRAM (CYD has no PSRAM)
//   - Smaller placeholder font (Styrene 20 vs 28)
...
// 20x20 grid scaled 10x → 200x200, centered on 320x240 panel (60px H margin,
// 20px V margin). 12x → 240x240 = 115KB doesn't fit in heap after NimBLE.
#define GRID         20
#define CELL         10
...
void splash_init(lv_obj_t *parent) {
    canvas_buf = (uint16_t*)heap_caps_malloc(CANVAS_W * CANVAS_H * 2, MALLOC_CAP_8BIT);
    if (!canvas_buf) {
    ...
    // Let taps pass through to splash_container's click handler — otherwise
    // the canvas swallows them and tap-to-cycle stops working on splash.
    ...
    lv_obj_set_style_text_font(label_status, &font_styrene_20, 0);
...
void splash_tick(void) {
    if (!active || SPLASH_ANIM_COUNT == 0 || !canvas_buf) return;
