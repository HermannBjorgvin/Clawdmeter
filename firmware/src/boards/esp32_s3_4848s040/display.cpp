...
// partial LVGL flush writes go straight into that framebuffer. That memory is
// cacheable, so every write must be followed by a cache writeback before the
// scan-out path can see it (see display_hal_draw_bitmap).
//
// setBrightness() is not available on RGB displays; backlight is PWM on GPIO 38.

// Declared in the ESP32-S3 ROM headers (Arduino_GFX declares it the same way).
...
void display_hal_init(void) {
    ...
    panel = new Arduino_ESP32RGBPanel(
        ...
        // Required: without them the LCD DMA fetches scanlines straight from
        // PSRAM, and a starved line FIFO makes the picture shift and roll.
...
void display_hal_draw_bitmap(int32_t x, int32_t y, int32_t w, int32_t h,
    ...
    if (!gfx) return;
    // memcpy row-by-row into the DMA framebuffer: ~0.5 ms for a 480x40 strip
    // against ~1.9 ms for the library's two-pixels-per-iteration word loop.
    ...
    // Mandatory with auto_flush=false and this direct write path: nothing else
    // flushes the framebuffer, and unwritten cache lines leave the panel
    // scanning stale pixels until they evict on their own.
    Cache_WriteBack_Addr((uint32_t)&fb[(size_t)y * LCD_WIDTH],
                         (uint32_t)(LCD_WIDTH * h * 2));
}
