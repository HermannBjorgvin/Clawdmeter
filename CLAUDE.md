...
- **Panel revision escape hatches.** Sunton ships this model with more than one panel/gamma. `-DCYD_ILI9341_TYPE2` selects the alternate init (TFT_eSPI's `ILI9341_2_DRIVER` equivalent) for washed-out colour. **Colour inversion is on by default** — verified on hardware, where the panel renders photo-negative without it; `-DCYD_NO_INVERT_COLORS` turns it off for revisions that don't need it.
