...
13. **macOS caches a bonded peripheral's GATT service table.** Adding/removing BLE characteristics (e.g. the permission-approval PERM_REQ/PERM_RESP chars) doesn't get picked up by an already-paired Mac — bleak/CoreBluetooth keeps using the old cached table and logs `Characteristic ... was not found!` on subscribe. Fix: toggle Bluetooth off/on (usually enough to force rediscovery), or forget & re-pair the device if that doesn't work. One-time after a characteristic-set change, not a recurring issue.
14. **A screen shown via `ui_show_*` while the panel is dimmed/asleep is invisible until something else wakes it.** `idle_note_activity()` only gets called from touch/button paths by default. Anything BLE-triggered and time-sensitive (see `ui_show_permission_request` in `ui.cpp`) must call it explicitly, or the content is correct but sits behind a black panel until an unrelated wake — reads as an inexplicable multi-minute "delay".
...
- **Permission-approval screen: physical Deny/Always/Allow for Claude Code's PreToolUse prompts (2026-09-15).** New PERM_REQ/PERM_RESP BLE characteristics (`...0005`/`...0006`), a dedicated `SCREEN_PERMISSION` in `ui.cpp` (top-level, not a usage_container sub-view, so its buttons never bubble into the tap-anywhere-to-splash handler), and `daemon/permission_hook.py` — a `PreToolUse` hook that relays through the usage daemon's new Unix socket (`~/.config/claude-usage-monitor/permission.sock`) rather than opening a second BLE connection. Fails to `ask` (never a silent allow/deny) on any hardware problem. Hit two gotchas along the way, now gotchas #13 and #14 above: macOS's cached GATT table after adding characteristics, and a permission screen rendering correctly behind a dimmed/sleeping panel because nothing called `idle_note_activity()` for a BLE-triggered event. Also added an audio driver for this board (ES8311, pins from the official Waveshare XiaoZhi BSP) and a chime on incoming requests. "Always Allow" persists as a real per-project `permissions.allow` rule in `.claude/settings.local.json` — the hook self-checks that file every call rather than depending on Claude Code noticing it mid-session; verified a repeat of the same command short-circuits with no device round trip at all.
...
- `...0005` PERM_REQ — host writes a pending tool-call approval request here (macOS daemon only currently; the Linux bash daemon doesn't implement this yet).
- `...0006` PERM_RESP — firmware notifies the tapped Deny/Always/Allow decision here.

**Permission-approval socket (macOS daemon, `claude_usage_daemon.py` only):** a Unix socket at `~/.config/claude-usage-monitor/permission.sock`, served by `asyncio.start_unix_server` alongside the main poll loop. `daemon/permission_hook.py` (a Claude Code `PreToolUse` hook, see the README's "Permission approval on the device" section) is a short-lived process per tool call — it connects, sends one JSON request line, gets one JSON reply line, exits. The daemon relays through whichever `Session` currently holds the live BLE connection (`_active_session` global, set/cleared in `connect_and_run`); with no connection, or on any BLE/parse failure, it answers `{"decision": "ask"}` so the hook always falls back safely to Claude Code's normal prompt rather than blocking or silently allowing.
