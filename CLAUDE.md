...
Plus one **unofficial community port** (not upstream-supported hardware):

- `boards/spotpear_s3_154/` — **SpotPear ESP32-S3-LCD-1.54-MUMA** (N16R8; a XiaoZhi "muma" AI toy board). Build env: `spotpear_s3_154`. **First non-AMOLED port**: display is a plain 4-wire **SPI ST7789 (240×240 IPS)**, not a QSPI AMOLED — `display.cpp` uses `Arduino_ESP32SPI` + `Arduino_ST7789` @ 40 MHz and drives brightness via **LEDC PWM on an ACTIVE-LOW backlight GPIO** (duty inverted; there's no panel brightness command). Touch is **CST816 @ 0x15** via the same inline FocalTech-style reader as the 1.8. Power/IMU/sound are stubs (`BOARD_HAS_BATTERY/IMU/SOUND=0`; no PMU). No rotation (`BOARD_HAS_ROTATION=0`). Lands on a dedicated **tiny** `compute_layout()` breakpoint (H<300): 240²-tuned fonts/panels, no header title (space reclaimed for the meters), compact `5h`/`7d` pills, parameterized in-panel fonts, thicker bars, and a plain-ASCII status line (`anim_plain` — the small font lacks the AMOLED spinner/ellipsis glyphs, which rendered as tofu boxes). Boots straight to the usage view via `-DBOOT_TO_USAGE`. **Verified pinout (from xiaozhi-esp32 board `sp-esp32-s3-1.54-muma` + on-hardware I2C scan — this board does NOT follow the Waveshare 1.54 pinmap despite SpotPear's wiki linking to it): LCD SCLK=4, MOSI=2, CS=5, DC=47, RST=38; backlight=GPIO 42 ACTIVE-LOW; touch I2C SDA=11/SCL=7, INT=12, RST=6; BOOT=GPIO 0.** Panel: ST7789, RGB order, color-inversion on (IPS=true → INVON). **Status: display VERIFIED on hardware — splash renders in correct color (framebuffer screenshot confirms Claude-orange). Flash ~20%, RAM ~31%.** Flashing over WSL needs `usbipd attach --auto-attach` (native-USB re-enumerates on every reset) + a udev rule (`SUBSYSTEM=="tty", ATTRS{idVendor}=="303a", MODE="0666"`) so the port stays accessible. PlatformIO lives in `~/.pio-venv` (venv). See `boards/spotpear_s3_154/TODO-PINS.md`. Env mirrors `waveshare_amoled_18` (S3, 16MB flash, `qio_opi` OPI PSRAM) minus the unused SensorLib/XPowersLib deps.
...
firmware/src/
  ...
  ui.{h,cpp}                — 3-screen UI (splash, usage, bluetooth). compute_layout() picks fonts/positions from board_caps() (responsive — THREE breakpoints: H>=460 → large/480², H>=300 → compact/368×448, else → tiny/240² for the SpotPear 1.54). The tiny branch adds small fonts, a parameterized `title_font`, `show_logo=false` (an 80×80 mark won't share the 240px top row), and `pair_l{1,2,3}_y` for the pairing-hint lines. Battery icon is now also hidden when `!board_caps().has_battery`.
