...
Eight ports today (two SoC families, five panel sizes):
...
- `boards/esp32_s3_4848s040/` — Guition ESP32-S3-4848S040 (ST7701 RGB parallel, 480×480 square, GT911 touch). Build env: `esp32_s3_4848s040`. Second RGB-panel port, and a generic board rather than a Waveshare one. **No button the firmware can see** (BOOT/RST inaccessible, GPIO 0 is display R4, SW1 is an IP5306 KEY pin) — touch is the only input. USB is a CH340 bridge, so the env needs `ARDUINO_USB_CDC_ON_BOOT=0`. No PMU/IMU; `has_battery = false`.
...
### 4848S040 — `esp32_s3_4848s040`
Generic Guition board; pin map and ST7701 init sequence come from OpenHASP, not a vendor datasheet.
- Display: **ST7701** 480×480 RGB parallel (DE=18, VSYNC=17, HSYNC=16, PCLK=21, R0-4=11/12/13/14/0, G0-5=8/20/3/46/9/10, B0-4=4/5/6/7/15); ST7701 init via SW SPI (CS=39, SCK=48, MOSI=47). Backlight is LEDC PWM on GPIO 38 — **1 kHz, not 100 Hz**: at 8-bit resolution 100 Hz overflows the S3's 18-bit LEDC divider.
- Touch: **GT911** via I2C (SDA=19, SCL=45), polled, no INT/RST wired. Probe 0x5D then 0x14.
- No IO expander, no AXP2101, no IMU. An **IP5306** power-bank PMIC is fitted but unread, so `has_battery = false` and `power_hal_is_vbus_in()` returns `true` (never auto-sleeps).
- **No button the firmware can see.** BOOT/RST are physically inaccessible, GPIO 0 is display R4, and SW1 is the IP5306's KEY pin (hardware power cut at ~10 s hold). Clear BLE bonds with `esptool erase_region 0x9000 0x5000` (the `nvs` partition).
- **USB is a CH340 bridge (`1A86:7523`) on UART0**; the S3's native USB is unwired, so the env sets `ARDUINO_USB_CDC_ON_BOOT=0` (see gotcha 14). The bridge drives auto-reset over DTR/RTS, which is why flashing works with BOOT/RST inaccessible.
- Free GPIOs for future use: 1, 2, 40, 41, 42. **43/44 are UART0 to the CH340** and **33-37 are octal PSRAM** — leave both alone.
...
firmware/src/
  ...
  boards/
    ...
    esp32_s3_4848s040/      — ST7701 RGB parallel + GT911, no PMU/IMU, no buttons
...
pio run -d firmware -e esp32_s3_4848s040                                        # build 4848S040 (S3, RGB TFT)
...
11. **RGB panels need bounce buffers** (LCD-4, 4848S040). `Arduino_RGB_Display` DMA-scans PSRAM. Pass `bounce_buffer_size_px = LCD_WIDTH * 10` so ESP-IDF allocates SRAM bounce buffers. Do not call `rgbpanel->getFrameBuffer()` after `gfx->begin()` — it constructs a second RGB panel and crashes.
...
13. **Writing straight into an RGB framebuffer needs a cache writeback.** It is cacheable PSRAM; Arduino_GFX ends its own draws with `Cache_WriteBack_Addr`, so a port that `memcpy`s into `getFramebuffer()` directly (and sets `auto_flush = false`) writes back nothing and the panel scans stale pixels until the lines evict. Write back the touched span after every strip. Bit the 4848S040.
14. **A board with a USB-UART bridge needs `ARDUINO_USB_CDC_ON_BOOT=0`.** With `=1`, `Serial` binds to `HWCDCSerial`, which discards every write when the native USB has no host — while IDF logging keeps flowing over UART0, so the board looks fine and only `Serial.print` vanishes. Check the port's VID:PID: `303A:*` is native USB, `1A86:*`/`10C4:*` a bridge. Bit the 4848S040.
